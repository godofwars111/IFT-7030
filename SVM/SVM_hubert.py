...
def extract_hubert_features(file_path):
    try:
        ...
        inputs = hubert_processor(audio_input, sampling_rate=16000, return_tensors="pt", padding=True)
       
        with torch.no_grad():
            hidden_states = hubert_model(inputs.input_values.to(device)).last_hidden_state
...
model_path = "svm_model_hubert.joblib"
